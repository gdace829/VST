...
def process_video(
    ...
    max_pixels: Optional[int] = None,  # 新增：最大像素限制
    min_pixels: Optional[int] = None,  # 新增：最小像素限制
) -> torch.Tensor:
    ...
    # --- 处理空间维度约束 (Spatial) ---
    # 如果字典中没有定义像素限制，则使用传入的参数
    if "max_pixels" not in video and max_pixels is not None:
        video["max_pixels"] = max_pixels
    
    if "min_pixels" not in video and min_pixels is not None:
