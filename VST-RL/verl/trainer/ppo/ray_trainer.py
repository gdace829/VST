...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        if self.config.recurrent.enable:
            self.train_dataset = self.recurrent_register.dataset_cls( # 
                ...
                prog_video=self.config.data.prog_video_length, # 渐进式视频长度
            )
        else:    
        ...
        val_batch_size = 4 # change for loader speed up
    ...
    def _validate(self):
        ...
        # =================================================================
        # 1. 多进程加载阶段 (Multi-worker Loading)
        # =================================================================
        print(f"Loading validation data from {len(self.val_dataloader)} batches...")
        all_protos = []
        
        # ✅ 核心修复：定义所有变长/复杂结构的字段
        # 根据你的 DataProto 分析，必须包含 prompt_ids, question_ids
        ragged_keys = [
            "raw_prompt_ids", "prompt_ids", "question_ids", 
            "multi_modal_inputs", "multi_modal_data", 
            "raw_prompt", "tools_kwargs"
        ]

        for test_data in tqdm(self.val_dataloader, desc="Loading Validation Data"):
            proto = DataProto.from_single_dict(test_data)
            
            # ✅ 强制转换：将变长字段转换为 (BatchSize,) 的 Object Array
            # 这能防止 numpy 尝试对齐内部维度 (如 78 vs 83)，从而解决 concatenate 报错
            for key in ragged_keys:
                if key in proto.non_tensor_batch:
                    val = proto.non_tensor_batch[key]
                    # 创建一个空的 object 数组，长度等于 batch_size
                    obj_arr = np.empty(len(val), dtype=object)
                    for i in range(len(val)):
                        obj_arr[i] = val[i] # 手动赋值，确保它是 0-dim 对象
                    proto.non_tensor_batch[key] = obj_arr
        ...
        # 合并所有小 Batch 为一个大 Batch
        if len(all_protos) > 1:
            test_batch_full = DataProto.concat(all_protos)
        else:
            test_batch_full = all_protos[0]
            
        # =================================================================
        # 2. 推理阶段 (Inference) - 只运行一次
        # =================================================================
        ...
        for test_batch in iterator:
            ...
            # =======================================================
            if self.config.recurrent.enable:
                # 注意：这里假设 val_dataset 也有 get_bactch_keys，如果没有请改回 manual list
            ...
            if "multi_modal_inputs" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.extend(["multi_modal_data", "multi_modal_inputs", "video_duration"])
            if "raw_prompt" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.append("raw_prompt")
            if "tools_kwargs" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.append("tools_kwargs")
            
            # 执行 pop，生成用于生成的 batch
    ...
    def init_workers(self):
        ...
        if self.config.recurrent.enable:
            if not self.async_rollout_mode:
                ...
                self.generation_manager = LLMGenerationManager( # 这里初始化了整个agent class
                    tokenizer=self.tokenizer,
                    processor=self.processor,
    ...
    def _save_checkpoint(self):
        ...
        curriculum_path = os.path.join(local_global_step_folder, "curriculum_frames.txt")
        if hasattr(self.train_dataset, 'prog_video') and self.train_dataset.prog_video:
            current_frames = self.train_dataset._read_frames()  # 从临时文件读取当前帧数
            with open(curriculum_path, 'w') as f:
                f.write(str(current_frames))
    ...
    def _load_checkpoint(self):
        ...
        curriculum_path = os.path.join(global_step_folder, "curriculum_frames.txt")
        if hasattr(self.train_dataset, 'prog_video') and self.train_dataset.prog_video:
            if os.path.exists(curriculum_path):
                with open(curriculum_path, 'r') as f:
                    frames = int(f.read().strip())
                self.train_dataset._write_frames(frames)  # 写入到临时文件 _sync_file
                print(f"Loaded curriculum frames: {frames} from {curriculum_path}")
            else:
                print(f"Curriculum frames file not found at {curriculum_path}, falling back to min_frames")
                self.train_dataset._write_frames(self.train_dataset.min_frames)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if hasattr(self.train_dataset, "set_step"):
                    self.train_dataset.set_step(self.global_steps)
                ...
                # with _timer('step', timing_raw):
                            ...
                            del gen_batch_output.non_tensor_batch['uid']
                            # breakpoint()
                    ...
                    #######
                    if self.config.trainer.balance_batch and not self.config.recurrent.enable: # 不进这个
                    ...
                    print(f"old_log_p_time:{timing_raw['old_log_prob']}")
                    # breakpoint()
                    ...
                    if self.use_reference_policy:
                        ...
                        print(f"ref_time:{timing_raw['ref']}")
