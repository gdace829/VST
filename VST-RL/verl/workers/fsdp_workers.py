...
class ActorRolloutRefWorker(Worker):
    ...
    def _process_multi_modal_inputs(self, data: DataProto):
        if "multi_modal_data" not in data.non_tensor_batch or "multi_modal_embeds" in data.non_tensor_batch:
            return

        ##TODO FIX FOR MULTI_MODAL --> MULTI_MODAL_EMBED
        
        assert len(data.non_tensor_batch["vid_uid"]) == data.batch.batch_size[0]

        if "vid_uid" in self._cache and (
            len(data.non_tensor_batch["vid_uid"]) != len(self._cache["vid_uid"]) or 
            not np.all(data.non_tensor_batch["vid_uid"] == self._cache["vid_uid"])
        ):
            self._cache.clear()
        ...
        if "multi_modal_inputs" not in self._cache:
            ...
            num_repeat_list = []
            vid_uids = data.non_tensor_batch["vid_uid"].tolist()
            for key, group in groupby(vid_uids):
                length = len(list(group))
                num_repeat_list.extend([length] * length)
                
            # 2. 遍历处理
            for i, multi_modal_data in enumerate(data.non_tensor_batch["multi_modal_data"]):
                # --- 核心修复逻辑开始 ---
                # 如果不是第一个元素，且和上一个元素的 UID 相同，说明是重复项，直接跳过
                # 因为我们在处理第一个元素时，已经一次性把后面重复的份数都加进去了
                if i > 0 and vid_uids[i] == vid_uids[i-1]:
                    continue
                
                # 获取当前这个视频组的重复次数
                current_repeat_num = num_repeat_list[i]
                # --- 核心修复逻辑结束 ---

                if "video" in multi_modal_data:
                    videos = multi_modal_data["video"]
                    if len(videos) != 0:
                        drop_keys = {'input_ids', 'attention_mask', 'second_per_grid_ts'}
                        
                        # 这里的 processor 操作比较耗时，我们只做一次
                        raw_outputs = self.processor(text=["<|video_pad|>"], videos=videos, return_tensors="pt")
                        
                        multi_modal_inputs = {k: v for k, v in raw_outputs.items() if k not in drop_keys}
                        ...
                        # 关键点：处理一次，然后复制 current_repeat_num 份
                        batch_multi_modal_inputs.extend([multi_modal_inputs] * current_repeat_num)
                    else:
                        # 如果 video 为空，也要占位，保持 batch 对齐
                        batch_multi_modal_inputs.extend([{}] * current_repeat_num)
                else:
                    # 如果没有 video 字段，同样占位
                    batch_multi_modal_inputs.extend([{}] * current_repeat_num)

            self._cache["vid_uid"] = data.non_tensor_batch["vid_uid"]
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_optimizer and data.meta_info.get('is_final_iter', True):
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
